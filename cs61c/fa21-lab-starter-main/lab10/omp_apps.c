...
#pragma omp parallel
    {
        int total_threads_nums = omp_get_num_threads();
        int current_thread_num = omp_get_thread_num();

        for (int j = current_thread_num; j < ARRAY_SIZE;
             j += total_threads_nums) {
            z[j] = x[j] + y[j];
        }
    }
}
...
#pragma omp parallel
    {
        int total_threads_nums = omp_get_num_threads();
        int current_thread_num = omp_get_thread_num();
        int chunk_size = ARRAY_SIZE / total_threads_nums;

        int begin = current_thread_num * chunk_size;
        int end = current_thread_num == total_threads_nums - 1
                      ? ARRAY_SIZE
                      : (current_thread_num + 1) * chunk_size;
