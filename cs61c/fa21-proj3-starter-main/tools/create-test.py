...
TRACE_HEADER = ["ra", "sp", "t0", "t1", "t2", "s0", "s1", "a0", "Requested_Address", "Requested_Instruction",
                "Time_Step"]
...
def generate_test_circ(asm_path, test_circ_path, slug, num_cycles):
    ...
    test_circ_data = test_circ_data.replace(HARNESS_IMPORT_REF,
...
def generate_output(asm_path, reference_output_path, slug, num_cycles=-1, is_pipelined=False):
    venus_cmd = ["java", "-jar", str(venus_path), str(asm_path), "--immutableText", "--trace", "--traceInstFirst",
                 "--tracepattern", TRACE_PATTERN, "--unsetRegisters"]
    if num_cycles > -1:
...
def create_test(asm_path, num_cycles=-1):
    ...
    try:
        ...
        detected_num_cycles = generate_output(asm_path, reference_output_2stage_path, slug, num_cycles=num_cycles,
                                              is_pipelined=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_path", help="Path to a RISC-V assembly file containing instructions for a test",
                        type=Path, nargs="+")
    parser.add_argument("-c", "--cycles", help="Number of simulation cycles to run (-1 for automatic)", type=int,
                        default=-1)
