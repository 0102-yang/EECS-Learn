    ...
    def run(self, pipelined=False):
        ...
        try:
            proc = subprocess.Popen(
                ["java", "-jar", str(logisim_path), "-tty", "table,binary,csv", str(self.circ_path)],
                stdout=subprocess.PIPE, encoding="utf-8", errors="ignore")

            with self.get_expected_table_path(pipelined=pipelined).open("r", encoding="utf-8",
...
if __name__ == "__main__":
    ...
    parser.add_argument("test_path", help="Path to a test circuit, or a directory containing test circuits", type=Path,
                        nargs="+")
    parser.add_argument("-p", "--pipelined",
