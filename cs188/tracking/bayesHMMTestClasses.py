...
class GraphEqualityTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if missingVars:
            self.addMessage('Student solution is missing variables: ' +
                            str(missingVars) + '\n')
        if extraVars:
            self.addMessage('Student solution has extra variables: ' +
                            str(extraVars) + '\n')
        studentEdges = set([
        ...
        if not sameValues:
            self.addMessage(
                'Student solution has incorrect values dictionary.')
            ...
            if missingDictVars:
                self.addMessage('Student dictionary is missing variables: ' +
                                str(missingDictVars))
            if extraDictVars:
                self.addMessage('Student dictionary has extra variables: ' +
                                str(extraDictVars))
            for variable, assignments in goldDict.items():
                ...
                if missing:
                    self.addMessage('Student dictionary for ' + variable +
                                    ' is missing assignments: ' + str(missing))
                if extra:
                    self.addMessage('Student dictionary for ' + variable +
                                    ' has extra assignments: ' + str(extra))
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for variable in goldNet.variablesSet():
            try:
                ...
                self.addMessage('Student Bayes net missing CPT for variable ' +
                                str(variable))
                return self.testFail(grades)
            goldFactor = goldNet.getCPT(variable)
            if not studentFactor == goldFactor:
                self.addMessage(
                    'First factor in which student answer differs from solution: P({} | {})'
                    .format(studentFactor.unconditionedVariables(),
        ...
        if studentFactor == goldenFactor:
            ...
            if self.alg == 'inferenceByVariableElimination':
                ...
                if self.callTrackingList != goldenCallTrackingList:
                    ...
                    self.addMessage('Correct order of operations: ' +
            ...
            studentProbabilityTotal = sum([
                studentFactor.getProbability(assignmentDict) for assignmentDict
                in studentFactor.getAllPossibleAssignmentDicts()
            ])
            correctProbabilityTotal = sum([
                goldenFactor.getProbability(assignmentDict) for assignmentDict
                in goldenFactor.getAllPossibleAssignmentDicts()
            ])
            if abs(studentProbabilityTotal - correctProbabilityTotal) > 10e-12:
                ...
                self.addMessage('Student sum of probability: ' +
                                str(studentProbabilityTotal))
                self.addMessage('Correct sum of probability: ' +
                                str(correctProbabilityTotal))
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        if self.constructRandomly:
            if self.alg == 'joinFactors' or self.alg == 'eliminate' or \
                ...
                replaceTestFile(self.testPath, "BayesNet",
        ...
        with open(filePath, 'w') as handle:
            ...
            if self.alg == 'inferenceByVariableElimination':
                handle.write('callTrackingList: "' +
                             repr(self.callTrackingList) + '"\n')
        return True
...
class BayesNetInputFactorEqualityTest(FactorEqualityTest):
    ...
    def __init__(self, question, testDict):
        super(BayesNetInputFactorEqualityTest,
        ...
        if self.alg == 'inferenceByVariableElimination':
            ...
            self.variableEliminationOrder = parseDict[
                'variableEliminationOrder']
    ...
    def solveProblem(self, moduleDict):
        ...
        if self.alg == 'inferenceByVariableElimination':
            ...
            studentComputation = studentComputationWithCallTracking(
                self.callTrackingList)
            solvedFactor = studentComputation(self.problemBayesNet,
                                              self.queryVariables,
                                              self.evidenceDict,
                                              self.variableEliminationOrder)
        elif self.alg == 'inferenceByLikelihoodWeightingSampling':
            randomSource = util.FixedRandom().random
            studentComputationRandomSource = getattr(inferenceModule,
                                                     self.alg + 'RandomSource')
...
class MostLikelyFoodHousePositionTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        super(MostLikelyFoodHousePositionTest,
              self).__init__(question, testDict)
        layoutText = testDict['layout']
        self.layoutName = testDict['layoutName']
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        studentBayesNet, _ = bayesAgentsModule.constructBayesNet(
            self.startState)
        ...
        studentPosition = studentFunction(
            self.evidence, studentBayesNet,
            self.eliminationOrder)[FOOD_HOUSE_VAR]
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        answer = staffFunction(self.evidence, staffBayesNet,
                               self.eliminationOrder)[FOOD_HOUSE_VAR]
        with open(filePath, 'w') as handle:
            handle.write(
                '# This is the solution file for %s.\n\nanswer: """\n' %
...
class VPITest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not correct:
            self.addMessage(
                'Student answer differed from solution by at least .0001')
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        answer = eval('agent.{}(self.evidence, self.eliminationOrder)'.format(
            self.targetFunction))
        with open(filePath, 'w') as handle:
            handle.write(
                '# This is the solution file for %s.\n\nanswer: """\n' %
...
def parseFactorInputProblem(testDict,
                            goingToEliminate=False,
                            goingToEvidenceAssign=False):
    parseDict = {}
    variableDomainsDict = {}
...
def replaceTestFile(file_path, typeOfTest, inputToTest):
    ...
    with open(abs_path, 'w') as new_file:
        ...
        if typeOfTest == 'BayesNet':
            ...
            new_file.write("\n" + "\n".join([
                factor.easierToParseString(prefix=name,
...
class DoubleInferenceAgentTest(testClasses.TestCase):
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        pac = InferenceAgent(inferenceFunction,
                             ghosts,
                             self.seed,
                             elapse=self.elapse,
                             observe=self.observe)
...
def run(layout_str,
    ...
    statTuple = (stats['wins'], len(games),
                 sum(stats['scores']) * 1.0 / len(games))
    if not quiet:
...
class InferenceAgent(bustersAgents.BustersAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.answerList.append(
            (self.numMoves, None, deepcopy(self.ghostBeliefs)))
            ...
            if abs(max(dist.values()) - max(refDist.values())) > .008:
                if self.errors == 0:
                    self.grades.fail(
                        ...
                        % (self.grades.currentQuestion, max(
                            dist.values()), max(refDist.values())))
                    self.errors += 1
...
class SeededRandomGhostAgent(Agent):
    ...
    def sample(self, distribution, values=None):
        if type(distribution) == util.Counter:
            items = [(k, distribution[k]) for k in fixed_order
