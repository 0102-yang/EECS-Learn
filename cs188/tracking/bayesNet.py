...
class BayesNet(object):
    ...
    def __init__(self, variables, inputInEdges, inputOutEdges,
                 inputVariableDomains):
        """
    ...
    def sameValuesDict(self, other):
        ...
        for variable, assignments in self.__variableDomainsDict.items():
            if sorted(other.__variableDomainsDict[variable]) != sorted(
                    assignments):
                return False
        return True
    ...
    def getCPTWithEvidence(self, variable, evidenceDict=None):
        ...
        if evidenceDict is None or len(evidenceDict.items()) == 0:
            ...
            reducedVariableDomains = self.getReducedVariableDomains(
                evidenceDict)
            variableCPT = self.getCPT(variable)
            return variableCPT.specializeVariableDomains(
                reducedVariableDomains)
    ...
    def getAllCPTsWithEvidence(self, evidenceDict=None):
        ...
        """
        return [
            self.getCPTWithEvidence(var, evidenceDict)
            for var in self.__variablesSet
        ]
        ...
        for CPT in self.getAllCPTsWithEvidence():
            ...
            returnStrings.append(
                CPT.easierToParseString(
                    prefix=prefix,
                    printVariableDomainsDict=printVariableDomainsDict))
...
class Factor(object):
    ...
    def __init__(self, inputUnconditionedVariables, inputConditionedVariables,
        ...
        products = list(
            itertools.product(*[
                inputVariableDomainsDict[variable]
    ...
    def __str__(self):
        ...
        varLine = " | " + " | ".join([
            str(unconditionedVariable)[:printSizeDict[unconditionedVariable]].
            center(printSizeDict[unconditionedVariable], ' ')
            for unconditionedVariable in self.__unconditionedVariables
        ])
        if len(self.__conditionedVariables) > 0:
            varLine += " | " + " | ".join([
                str(conditionedVariable)[:printSizeDict[conditionedVariable]].
        ...
        if len(self.__conditionedVariables) == 0:
            returnString += " " + "".join(
                ["-" for _ in range(varLineLength - 1)]) + "\n"
        ...
        for assignmentDict in self.getAllPossibleAssignmentDicts():
            ...
            if probability is None:
                ...
                formattedProb = "%.1e" % probability if probability < 10e-2 else (
                    "%." + str(8 - digits) + "f") % probability
    ...
    def __repr__(self):
        returnRepr = "Factor("
        initArgs = [
            self.__unconditionedVariables, self.__conditionedVariables,
                ...
                for value in domain:
                    if value not in oldVariableDomain:
                        raise ValueError(
                            ...
                            str(self.variableDomainsDict()) + " variable: " +
                            str(variable) + " value: " + str(value))

        newFactor = Factor(self.unconditionedVariables(),
                           self.conditionedVariables(), newVariableDomainsDict)

        for assignmentDict in newFactor.getAllPossibleAssignmentDicts():
            newFactor.setProbability(assignmentDict,
                                     self.getProbability(assignmentDict))
...
def constructEmptyBayesNet(variableList, edgeTuplesList, variableDomainsDict):
    ...
    newBayesNet = BayesNet(variablesSet, inEdges, outEdges,
                           variableDomainsDict)
    return newBayesNet
...
def constructRandomlyFilledBayesNet(variableList, edgeTuplesList,
                                    variableDomainsDict):
    " Random Bayes net constructor "
    bayesNet = constructEmptyBayesNet(variableList, edgeTuplesList,
                                      variableDomainsDict)
...
def fillTablesRandomly(bayesNet):
    ...
    for variable in bayesNet.variablesSet():
        ...
        CPT = constructAndFillFactorRandomly([variable],
                                             conditionedVariablesList,
                                             bayesNet.variableDomainsDict())
        bayesNet.setCPT(variable, CPT)


def fillWithOneConditionedAssignmentRandomly(factor, unconditionedVariables,
                                             conditionedVariables, product,
                                             variableDomainsDict):
    """ 
    ...
    for (randomFill,
         variableValue) in zip(randomFills,
                               cartesianProductOfUnConditionalAssignments):
        assignmentDict = dict(
            zip(
                list(unconditionedVariables) + list(conditionedVariables),
                list(variableValue) + list(product)))
        factor.setProbability(assignmentDict,
                              randomFill / conditionalProbabilitySum)


def constructAndFillFactorRandomly(unconditionedVariables,
    ...
    if len(conditionedVariables) > 0:
        ...
        for product in cartesianProductOfConditionalAssignments:
            fillWithOneConditionedAssignmentRandomly(newFactor,
        ...
        if variable in newVariables:
            ...
            if variable in evidenceVariables and \
                ...
                if len(removedVariablesParents) == 0:
                    newCPT = oldCPT.specializeVariableDomains(
                        newVariableDomainsDict)
                else:
...
def printStarterBayesNet():
    ...
    trafficCPT = Factor(['Traffic'], ['Raining', 'Ballgame'],
                        variableDomainsDict)
    ...
    for conditionedVariable in factor.conditionedVariables():
        if len(variableDomainsDict[conditionedVariable]) > 1:
            ...
            raise ValueError("The factor to be normalized must have only one " + \
                            ...
                            str(factor))
    newUCVars, newCVars = factor.unconditionedVariables(
