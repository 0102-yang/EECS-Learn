...
def drawNullSquare(grid, x, y, isObstacle, isTerminal, isCurrent):
    ...
    if isTerminal and not isObstacle:
        square((screen_x, screen_y),
               ...
               filled=0,
               width=2)
        text((screen_x, screen_y), TEXT_COLOR, str(grid[x][y]), "Courier", -24,
             "bold", "c")

    text_color = TEXT_COLOR

    if not isObstacle and isCurrent:
        circle((screen_x, screen_y),
               0.1 * GRID_SIZE,
               LOCATION_COLOR,
...
def drawSquare(x, y, val, min, max, valStr, action, isObstacle, isTerminal,
    ...
    if not isObstacle:
        text((screen_x, screen_y), text_color, valStr, "Courier", -30, "bold",
             "c")
...
def square(pos, size, color, filled, width):
    x, y = pos
    dx, dy = size, size
    return polygon([(x - dx, y - dy), (x - dx, y + dy), (x + dx, y + dy),
                    (x + dx, y - dy)],
                   outlineColor=color,
                   fillColor=color,
                   filled=filled,
