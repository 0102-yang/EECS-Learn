        ...
        if use_graphics:
            ...
            for i in range(10):
                ...
                for j, (prob, image) in enumerate(
                        zip(probs[indices],
                            dev_images[dev_labels == i][indices])):
...
class ReplayMemory(object):
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        self.memory[self.cursor] = Transition(state, action, reward,
                                              next_state, done)
...
def get_data_and_monitor_online_rl(model, target_model, agent, env):
    ...
    for episode in range(n_episode):
        ...
        while not done:
            ...
            next_state = s2 if not done else (
                -1, -1)  # define terminal state to be -1, -1
            action_num = possible_action_list.index(a)
            reward = r if r is not None else 0
            print("(s, action_num, reward, next_state, done)",
        ...
        if (episode + 1) % episode_print_interval == 0:
            print(
                "[Episode: {:3}] Reward: {:5} Mean Reward of last {} episodes: {:5.1f} epsilon: {:5.2f}"
                .format(episode + 1, total_reward, num_episodes_to_average,
                        np.mean(rewards), eps))
