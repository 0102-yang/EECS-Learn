...
def parseOptions():
    ...
    optParser.add_option(
        ...
        default="BookGrid",
        help=
        'Grid to use (case sensitive; options are BookGrid, BridgeGrid, CliffGrid, MazeGrid, default %default)'
    )
    optParser.add_option(
        '-w',
        '--windowSize',
        ...
        default=150,
        help=
        'Request a window width of X pixels *per grid cell* (default %default)'
    )
    optParser.add_option(
        '-a',
        '--agent',
        ...
        default="random",
        help=
        'Agent type (options are \'random\', \'value\', \'q\', and \'learn\', default %default)'
    )
    ...
    optParser.add_option(
        ...
        default=1.0,
        help=
        'Speed of animation, S > 1.0 is faster, 0.0 < S < 1.0 is slower (default %default)'
    )
...
if __name__ == '__main__':
    ...
    try:
        if not opts.manual and opts.agent in ('value', 'asynchvalue',
                                              'priosweepvalue', 'learn'):
            if opts.valueSteps:
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent in ('random', 'value', 'asynchvalue',
                              'priosweepvalue'):
