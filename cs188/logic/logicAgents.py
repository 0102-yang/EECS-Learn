    ...
    def __init__(self,
        ...
        if fn not in dir(plan_mod):
            raise AttributeError(
                fn + ' is not a planning function in logicPlan.py.')
        ...
        if prob not in globals().keys() or not prob.endswith('Problem'):
            raise AttributeError(
                prob + ' is not a planning problem type in logicAgents.py.')
...
class CheckSatisfiabilityAgent(LogicAgent):
    ...
    def __init__(self,
                 fn='checkLocationSatisfiability',
                 prob='LocMapProblem',
                 plan_mod=logicPlan):
        # Warning: some advanced Python magic is employed below to find the right functions and problems
...
class LocalizeMapAgent(LogicAgent):
    ...
    def addNoOp_t0(self):
        self.visited_states = [self.visited_states[0]] + list(
            self.visited_states)
    ...
    def get_known_walls_non_walls_from_known_map(self, known_map):
        ...
        if known_map == None:
            raise Exception(
                'Student code supplied None instead of a 2D known map')
        known_walls = [[(True if entry == 1 else False) for entry in row]
                       for row in known_map]
        known_non_walls = [[(True if entry == 0 else False) for entry in row]
                           for row in known_map]
...
class LocalizationLogicAgent(LocalizeMapAgent):
    ...
    def __init__(self,
                 fn='localization',
                 prob='LocalizationProblem',
                 plan_mod=logicPlan,
                 display=None,
                 scripted_actions=[]):
        super(LocalizationLogicAgent, self).__init__(fn, prob, plan_mod,
                                                     display, scripted_actions)
    ...
    def getAction(self, state):
        ...
        if i < self.num_timesteps:
            ...
            if planning_fn_output == None:
                raise Exception(
                    'Studenct code supplied None instead of result')
            if isinstance(self.display, graphicsDisplay.PacmanGraphics):
                self.drawPossibleStates(planning_fn_output,
    ...
    def drawPossibleStates(self,
        ...
        self.display.colorCircleCells(possibleLocations,
                                      direction=direction,
...
class MappingLogicAgent(LocalizeMapAgent):
    ...
    def __init__(self,
                 ...
                 display=None,
                 scripted_actions=[]):
        super(MappingLogicAgent, self).__init__(fn, prob, plan_mod, display,
                                                scripted_actions)
    ...
    def getAction(self, state):
        ...
        if i < self.num_timesteps:
            ...
            planning_fn_output = next(self.planning_fn_output)
            if isinstance(self.display, graphicsDisplay.PacmanGraphics):
                self.drawWallBeliefs(planning_fn_output, self.actions[i],
...
class SLAMLogicAgent(LocalizeMapAgent):
    ...
    def __init__(self,
                 ...
                 display=None,
                 scripted_actions=[]):
        super(SLAMLogicAgent, self).__init__(fn, prob, plan_mod, display,
                                             scripted_actions)
        self.scripted_actions = scripted_actions
        self.num_timesteps = len(
            self.scripted_actions) if self.scripted_actions else 10
        self.live_checking = True
    ...
    def getAction(self, state):
        ...
        if proposed_action not in self.getValidActions(
                pacman_loc) and proposed_action not in ["Stop", "EndGame"]:
    ...
    def getPercepts(self):
        ...
        west_iswall = self.problem.walls[x - 1][y]
        num_adj_walls = sum(
            [north_iswall, south_iswall, east_iswall, west_iswall])
...
class PositionPlanningProblem(logicPlan.PlanningProblem):
    ...
    def __init__(self,
        ...
        if warn and (gameState.getNumFood() != 1
            ...
            print(
                'Warning: this does not look like a regular position planning maze'
            )
