...
class Expr:
    ...
    def __init__(self, op, *args):
        ...
        if not args and not is_prop_symbol(self.op):
            raise SyntaxError(
                "Unacceptable symbol base name (%s). Name must start with an upper-case alphabetic character that and is not TRUE or FALSE. Furthermore, only the following are allowed: capital and lower case alphabetic, 0-9, _, \",\", [, and ]."
                % self.op)
    ...
    def __init__(self, sym_str: str, *index: Tuple[int], time: int = None):
        ...
        if sym_str in self.all_checked:
            if sym_str in self.double_index:
                if len(index) != 2:
                    raise SyntaxError("Unexpected " + sym_str +
                                      " Symbol. Was expecting 2 coordinates.")
            elif len(index) != 0:
                raise SyntaxError("Unexpected " + sym_str +
                                  " Symbol. Was expecting 0 coordinates.")
            if sym_str in self.time_index:
                if time == None:
                    raise SyntaxError("Unexpected " + sym_str +
                                      " Symbol. Was expecting time stamp.")
            elif time != None:
                raise SyntaxError("Unexpected " + sym_str +
                                  " Symbol. Was expecting no time stamp.")
        ...
        if len(index) > 0:
            if len(index) > 4:
                raise SyntaxError(
                    "Too many arguments to SymbolExpr constructor. SymbolExpr(symbol_str, [index1], [index2], [index3], [index4], time=[time]), or fewer indicies -- possibly 0."
                )
            if len(index) == 1:
...
def is_prop_symbol(s):
    ...
    ) and s != 'TRUE' and s != 'FALSE' and re.match(r'[a-zA-Z0-9_\[\],]*$', s)
...
def prop_symbols(x):
    ...
    if not isinstance(x, Expr):
        ...
        return list(
            set(symbol for arg in x.args for symbol in prop_symbols(arg)))
...
def conjoin(exprs, *args):
    ...
    for expr in exprs:
        assert isinstance(expr,
                          Expr), "An item in list to conjoin is not an Expr."
...
def disjoin(exprs, *args):
    ...
    for expr in exprs:
        assert isinstance(expr,
                          Expr), "An item in list to disjoin is not an Expr."
    ...
    for c in clauses:
        ...
        for lit in literals:
            ...
            if not is_symbol(symbol_str):
                ...
                print("The symbol %s must begin with an upper-case letter." %
                      symbol_str)
                return False
