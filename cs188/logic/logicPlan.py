...
def plTrueInverse(assignments: Dict[Expr, bool],
                  inverse_statement: Expr) -> bool:
...
def pacmanSuccessorAxiomSingle(x: int,
                               y: int,
                               time: int,
                               walls_grid: List[List[bool]] = None) -> Expr:
    """
...
def SLAMSuccessorAxiomSingle(x: int, y: int, time: int,
                             walls_grid: List[List[bool]]) -> Expr:
    """
    ...
    for direction in DIRECTIONS:
        ...
        auxilary_expression_definitions.append(wall_dir_combined_literal %
                                               wall_dir_clause)
...
def sensorAxioms(t: int, non_outer_wall_coords: List[Tuple[int, int]]) -> Expr:
    ...
    for direction in DIRECTIONS:
        ...
        for x, y in non_outer_wall_coords:
            combo_var = PropSymbolExpr(pacman_wall_str,
                                       x,
                                       y,
                                       x + dx,
                                       y + dy,
            ...
            combo_var_def_exprs.append(
                combo_var % (PropSymbolExpr(pacman_str, x, y, time=t)
...
def SLAMSensorAxioms(t: int, non_outer_wall_coords: List[Tuple[int,
                                                               int]]) -> Expr:
    all_percept_exprs = []
    combo_var_def_exprs = []
    for direction in DIRECTIONS:
    ...
    for n in range(1, 4):
        ...
        n_walls_blocked_sent = disjoin([
            conjoin([
                PropSymbolExpr(blocked_str, time=t)
        ...
        percept_to_blocked_sent.append(
            PropSymbolExpr(geq_num_adj_wall_str_map[n], time=t) %
...
def allLegalSuccessorAxioms(
        t: int, walls_grid: List[List],
        non_outer_wall_coords: List[Tuple[int, int]]) -> Expr:
...
def SLAMSuccessorAxioms(t: int, walls_grid: List[List],
                        non_outer_wall_coords: List[Tuple[int, int]]) -> Expr:
    """walls_grid can be a 2D array of ints or bools."""
    all_xy_succ_axioms = []
    for x, y in non_outer_wall_coords:
