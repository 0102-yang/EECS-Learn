...
class EvalTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.preamble = compile(testDict.get('preamble', ""),
                                "%s.preamble" % self.getPath(), 'exec')
        self.test = compile(testDict['test'], "%s.test" % self.getPath(),
                            'eval')
...
class PacphysicsTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        pacphysics_axioms = logicPlan.pacphysicsAxioms(self.t, all_coords,
                                                       non_outer_wall_coords,
                                                       walls_list,
                                                       self.sensorAxioms,
                                                       self.successorAxioms)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        gold_soln_clauses_list_being_conjoined = str(
            gold_solution)[1:-1].split(" & ")
        ...
        for soln_clause in soln_clauses_list_being_conjoined:
            if "<=>" in soln_clause:
                if self.axiom_type == 'sensor':
                    ...
                    break
            contains_open_parens = ("(" in soln_clause[1:-1]) or (
                "(" in soln_clause[1:-1])
            if contains_open_parens:
                grades.addMessage('FAIL: {}'.format(self.path))
                grades.addMessage(
                    '\tStudent solution does not combine sentences properly.')
                grades.addMessage(
                    '\tMake sure you append the items to join with "and",'
                    ' and conjoin at the end.')
                return False

        # Check number of clauses is correct.
        gold_soln_num_clauses_conjoined = len(
            gold_soln_clauses_list_being_conjoined)
        ...
        if gold_soln_num_clauses_conjoined != soln_num_clauses_conjoined:
            grades.addMessage('FAIL: {}'.format(self.path))
            grades.addMessage(
                '\tStudent solution differed from autograder solution')
            grades.addMessage(
                '\tNumber of clauses being conjoined in student solution: {}'.
                format(soln_num_clauses_conjoined))
            grades.addMessage(
                '\tNumber of clauses being conjoined in correct solution: {}'.
                format(gold_soln_num_clauses_conjoined))
            return False
        ...
        for gold_clause in gold_soln_clauses_list_being_conjoined:
            if gold_clause not in soln_clauses_list_being_conjoined:
                grades.addMessage('FAIL: {}'.format(self.path))
                grades.addMessage(
                    '\tStudent solution does not contain clause {}'.format(
                        gold_clause))
                return False

        if set(soln_clauses_list_being_conjoined) != set(
                gold_soln_clauses_list_being_conjoined):
            grades.addMessage('FAIL: {}'.format(self.path))
            grades.addMessage(
                '\tStudent solution differed from autograder solution on clause set comparison'
            )
        ...
        if sorted(str(solution)) != sorted(str(gold_solution)):
            grades.addMessage('FAIL: {}'.format(self.path))
            grades.addMessage(
                '\tStudent solution differed from autograder solution on character list comparison'
            )
            grades.addMessage('\tStudent solution: {}'.format(solution))
            grades.addMessage('\tCorrect solution: {}'.format(gold_solution))
            return False
...
class LocationSatisfiabilityTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        pac = logicAgents.CheckSatisfiabilityAgent(
            'checkLocationSatisfiability', 'LocMapProblem', logicPlan)
        ghosts = []
        ...
        games = next(
            pacman.runGames(lay,
                            ...
                            timeout=180))
        loc_sat_models = logicPlan.checkLocationSatisfiability(
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for i, solution_i in enumerate(solution):
            ...
            if gold_solution_i == "False" and solution_i != "False":
                grades.addMessage('FAIL: {}'.format(self.path))
                grades.addMessage(
                    '\tStudent solution differed from autograder solution for {}'
                    .format(self.soln_labels[i]))
                grades.addMessage(
                    '\tStudent model found satisfiable solution but no satisfiable solution exists.'
                )
                return False
                ...
                grades.addMessage(
                    ...
                    .format(self.soln_labels[i]))
                grades.addMessage(
                    '\tStudent model found no satisfiable solution when a satisfiable solution exists.'
                )
                return False
            elif gold_solution_i == "False" and solution_i == "False":
                continue
            ...
            gold_solution_i_str_pairs_list = gold_solution_i[2:-2].split(
                "), (")
            gold_solution_i_tuples_list = [
                tuple(pair.split(", "))
                for pair in gold_solution_i_str_pairs_list
            ]
            gold_solution_i_dict = dict(gold_solution_i_tuples_list)
            solution_i_str_pairs_list = solution_i[2:-2].split("), (")
            solution_i_tuples_list = [
                tuple(pair.split(", ")) for pair in solution_i_str_pairs_list
            ]
            ...
            if gold_solution_i_num_vars != solution_i_num_vars:
                ...
                grades.addMessage(
                    '\tStudent solution differed from autograder solution')
                grades.addMessage(
                    '\tNumber of variables in student solution: {}'.format(
                        solution_i_num_vars))
                grades.addMessage(
                    '\tNumber of variables in correct solution: {}'.format(
            ...
            for gold_solution_var in gold_solution_i_dict:
                if gold_solution_var not in solution_i_dict:
                    grades.addMessage('FAIL: {}'.format(self.path))
                    grades.addMessage(
                        '\tStudent solution does not contain the same variables as correct solution'
                    )
                    grades.addMessage(
                        '\tCorrect solution variable missing in student solution: {}'
                        .format(gold_solution_var))
                    return False
            ...
            for key in gold_solution_i_dict:
                if gold_solution_i_dict[key] != solution_i_dict[key]:
                    grades.addMessage('FAIL: {}'.format(self.path))
                    grades.addMessage(
                        '\tStudent model does not assign the correct value for variable {}'
                        .format(key))
                    grades.addMessage('\tStudent value for {}: {}'.format(
                        key, solution_i_dict[key]))
                    grades.addMessage('\tCorrect value for {}: {}'.format(
                        key, gold_solution_i_dict[key]))
                    if "WALL" in key:
                        grades.addMessage(
                            '\tDouble check that you are loading the map properly.'
                        )
                    return False
            ...
            if str(solution_i) != str(gold_solution_i):
                ...
                grades.addMessage(
                    '\tCorrect solution: {}'.format(gold_solution_i))
                return False
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        for i, solution_i in enumerate(solution):
            handle.write('{}: "{}"\n'.format(
                self.soln_labels[i], logicPlan.modelToString(solution_i)))
        handle.close()
...
class PositionProblemTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        lay = layout.Layout([l.strip() for l in self.layoutText.split('\n')])
        pac = logicAgents.LogicAgent('plp', 'PositionPlanningProblem',
                                     logicPlan)
        ghosts = []
        disp = textDisplay.NullGraphics()
        games = next(
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not solution[0] or solution[1] < gold_score:
            ...
            if solution[0]:
                ...
                result_str = "loses"
            grades.addMessage('\tstudent solution result: Pacman %s' %
                              result_str)
            grades.addMessage('\tstudent solution score: %d' % solution[1])
            grades.addMessage('\tstudent solution path: %s' %
                              ' '.join(solution[2]))
...
class LocalizationProblemTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        if isinstance(disp, graphicsDisplay.PacmanGraphics
                      ):  # autograder.py has incorrect options
        ...
        yield from pacman.runGames(lay,
                                   pac,
                                   ghosts,
                                   disp,
                                   1,
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for t, solution in enumerate(self.solution(logicPlan)):
            ...
            if set(solution) != set(gold_solution[t]):
                grades.addMessage('FAIL: {}'.format(self.path))
                grades.addMessage(
                    '\tStudent solution differed from autograder solution at timestep t = {}'
                    .format(t))
                grades.addMessage(
                    '\tStudent solution at time t = {}: {}'.format(
                        t, solution))
                grades.addMessage(
                    '\tCorrect solution at time t = {}: {}'.format(
                        t, gold_solution[t]))
                return False
        ...
        if num_timesteps != len(gold_solution):
            ...
            grades.addMessage(
                '\tStudent solution differed from autograder solution')
            grades.addMessage(
                '\tStudent solution timestep number: {}'.format(num_timesteps))
            grades.addMessage('\tCorrect solution timestep number: {}'.format(
                len(eval(solutionDict['possible_locations_per_timestep']))))
            return False
...
class MappingProblemTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        if isinstance(disp, graphicsDisplay.PacmanGraphics
            ...
            disp = graphicsDisplay.PacmanGraphics(
                frameTime=0.5, render_walls_beforehand=False)
    ...
    def check_len(self, grades, soln, gold_soln, str_info=""):
        if len(soln) != len(gold_soln):
            grades.addMessage('FAIL: {}'.format(self.path))
            grades.addMessage('\tstudent solution length {}: {}'.format(
                str_info, len(soln)))
            grades.addMessage('\tcorrect solution length {}: {}'.format(
                str_info, len(gold_soln)))
            return False
        return True
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for t, solution_t in enumerate(self.solution(logicPlan)):
            if solution_t == None:
                num_timesteps = t
                break
            if not self.check_len(grades, solution_t, gold_solution[t],
                                  "at time t = {}".format(t)):
                return False
        ...
        if num_timesteps != len(gold_solution):
            ...
            grades.addMessage(
                '\tStudent solution timestep number: {}'.format(num_timesteps))
            grades.addMessage('\tCorrect solution timestep number: {}'.format(
                len(eval(solutionDict[self.solution_label]))))
            return False
...
class SLAMProblemTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.solution_labels = [
            'known_map_per_timestep', 'possible_locations_per_timestep'
        ]
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for t, solutions_at_t in enumerate(self.solution(logicPlan)):
            if solutions_at_t is None:
                num_timesteps = t
                break
            for soln_label, solution in zip(self.solution_labels,
                                            solutions_at_t):
                ...
                if solution != gold_solution[t]:
                    ...
                    grades.addMessage(
                        ...
                        .format(t))
                    grades.addMessage(
                        '\tStudent solution for {} at time t = {}: {}'.format(
                            soln_label, t, solution))
                    grades.addMessage(
                        '\tCorrect solution for {} at time t = {}: {}'.format(
                            soln_label, t, gold_solution[t]))
                    return False
        ...
        if num_timesteps != len(eval(solutionDict[self.solution_labels[0]])):
            ...
            grades.addMessage(
                '\tStudent solution timestep number: {}'.format(num_timesteps))
            grades.addMessage('\tCorrect solution timestep number: {}'.format(
                len(eval(solutionDict[self.solution_labels[0]]))))
            return False
...
class LogicStatementTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if str(truths) == solutionDict['result']:
            ...
            # grades.addMessage('\t%s' % self.failure)
            grades.addMessage(
                'Your solution\'s first error occurred on model %d.' %
                firstError)
            grades.addMessage('MODEL:  %s' % model)
            grades.addMessage(
                'The correct answer is %s but you returned %s.' %
                (solution_truths[firstError - 1], truths[firstError - 1]))
